...
all_data$britishColumbia |> plot_hist(vline = tibble(lice = 3), xlim = c(0, 5), xlab = "Mean number of motile lice per fish")
...
all_data$norway |> 
  filter(year < 2017) |> 
  plot_hist(vline = tibble(lice = 0.5), facet = "year", xlim = c(0, 2))
```
Norway 2012 -- 2016
:::
...
all_data$norway |> 
  filter(!limit == "Ukjent") |> 
  filter(year >= 2017) |> 
  plot_hist(vline = tibble(lice = c(0.2, 0.5), limit = c("0.2", "0.5")), facet = "seasonyear", xlim = c(0, 1))
```
Norway since 2017
:::
