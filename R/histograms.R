...
make_histograms <- function(all_data){
  ...
  norway <- all_data$norway |>
    mutate(group = case_when(
      ...
      between(year, 2017, 2024) & limit == 0.5 ~ "Norway 2017-2024 outside smolt migration",
  ...
  plot_data <- bind_rows(scotland, norway) |>
    mutate(group = factor(group, levels = c("Scotland 2021-2023", "Norway 2012", "Norway 2013-2016", "Norway 2017-2024 during smolt migration", "Norway 2017-2024 outside smolt migration")))


  lice_hist <- plot_data |>
    filter(group %in% c("Scotland 2021-2023", "Norway 2012", "Norway 2013-2016", "Norway 2017-2024 outside smolt migration")) |>
    filter(lice > 0) |>
    ggplot(aes(x = lice)) +
    ...
    labs(x = "Mean number of adult female lice per fish", ylab = "Number of surveys")
