...
run_rdd <- function(all_data) {
  rdds <- list(
    ...
    britishColumbia = rdd_wrap(
      ...
      step = 0.02,
      include_zero = FALSE,
      xlabel = "Number of Motile Lice"
    ),
    ireland = rdd_wrap(
      all_data$ireland |> filter(limit == 0.5),
      lice,
      0.5,
      xlim = c(0.2, 0.8),
      step = 0.01,
      include_zero = FALSE,
      xlabel = "Gravid female lice"
    )
  )
  ...
  norway0.5 <- set_names(2012:2024) |>
    map(
      \(y)
        rdd_wrap(
          all_data$norway |> filter(year == y, limit != 0.2),
          lice,
          0.5,
          xlim = c(0.2, 0.8),
          include_zero = FALSE
...
extract_stats_text <- function(t_stats) {
  stats_text <- t_stats |>
    mutate(
      T = round(T, 2),
      p = if_else(p >= 0.001, formatC(p, digits = 2, format = "fg"), "< 0.001")
    ) |>
    purrr::transpose() |>
