...
Open net pen salmon farms release pharmaceutical products [@CasaResino2021], heavy metals [@Burridge2010], and organic waste # into the environment.
...
Salmon lice released by the aquaculture industry are thought to be one of the drivers of decreasing wild salmon populations# alongside climate change xxx #.
...
In this paper, we explore how compliance with louse regulations by the aquaculture industry is affected by the severity of the regulations and whether the louse counts are self-reported or conducted by the regulator.
...
We then compare the prevalence of non-compliance with the severity of the regulations.
...
The proportion of lice counts that are comply with the regulations [@fig-compliance] is high and increasing in Ireland and Norway, but somewhat lower and more varied in British Columbia.
...
The Ireland data are especially interesting as the regulator is responsible for counting the lice #. 
In British Columbia, facilities are regularly audited to ensure that the counts are performed correctly #.
...
In a previous case [@NorgesHoeyesterett2015], a manager of a company was convicted of reporting that there were no lice when counts had not been carried out.
...
In Norway, the Traffic Light System is a regulatory tool used since 2017 to balance the growth of the aquaculture industry with the need to protect wild salmon populations #.
...
If the count data do not reflect the true number of lice, estimates of lice infection pressure will be biased.
...
As female salmon lice are monogamous [@Ritchie1996] and able to store sperm for several egg strings #, the probability that a louse is reproductively active will not depend on the current lice density, but on the highest density it experienced as an adult.
...
The British Columbia and Irish lice counts does not show a discontinuity at the regulatory limit. 
In these jurisdictions, lice counts are either audited regularly or performed by the regulator.
...
Counts of salmon lice from Norwegian aquaculture facilities do not fully reflect the true number of lice.
