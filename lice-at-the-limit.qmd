...
_Lepeophtheirus salmonis_, the salmon louse, is a ectoparasitic copepod that infests salmonoid fish causing welfare problems# and economic losses# in the aquaculture industry and is one of the drivers of decreasing wild salmon populations#. 
Consequently, countries with salmon farming have established regulatory frameworks, focused on the mean number of adult lice per fish [@tbl-regulations], to control lice infestations [#vormedal2023].
...
[#Jeong2023] reported that in Norway, but not in British Columbia, the distribution of louse counts has a "cliff" at the regulatory limit.
Here, we further examine this cliff or discontinuity in the louse counts distribution at the regulatory limit and try to explain it.
...
britishcolumbia <- read_delim("data/british-columbia/lice-count-dens-pou-2011-ongoing-rpt-pac-dfo-mpo-aquaculture-eng.csv", na = "n/a") |> 
  janitor::clean_names()
```
...
period <- list(
  britishcolumbia = glue("{min(britishcolumbia$year)}--{max(britishcolumbia$year)}"),
  scotland = glue("{min(scotland$year)}--{max(scotland$year)}"),
  norway = glue("{min(norway$ar)}--{max(norway$ar)}")
)
...
Lice count data were downloaded from the regulators in British Columbia, (Canada) (`r period$britishcolumbia`), Scotland (`r period$scotland`), and Norway (`r period$norway`).
...
#| label: run-rdd
rdds <- list(
  scotland = rdd_wrap(scotland2, adult_female, 2, xlim = c(1.5, 2.5), step = 0.02), #
  britishcolumbia = rdd_wrap(britishcolumbia, average_l_salmonis_motiles_per_fish, 3, xlim = c(2.5, 3.5), step = 0.02, xlabel = "Number of Motile Lice")
  )

norway0.5_rdd <- set_names(2012:2024) |> 
  map(\(y) rdd_wrap(norway2 |> filter(year == y, limit != 0.2), adult_female, 0.5, xlim = c(0.2, 0.8)))

norway0.2_rdd <- set_names(2017:2024) |> 
  map(\(y) rdd_wrap(norway2 |> filter(year == y, limit == 0.2), adult_female, 0.2, xlim = c(0.1, 0.3))
  )
```
...
t_stat <- map(rdds, "jump") |> 
  map(rdd_extract) |> 
  list_rbind(names_to = "wherewhen") 

norway_t <- bind_rows(
`0.5`= map(norway0.5_rdd, "jump") |> 
  map(rdd_extract) |> 
  list_rbind(names_to = "year"), 

`0.2` = map(norway0.2_rdd, "jump") |> 
  map(rdd_extract) |> 
  list_rbind(names_to = "year"), 
.id = "limit"
) |> 
...
stats_text <- bind_rows(
    t_stat,
    norway_t |> filter(limit == 0.5, year %in% c(2012, 2020)) |> mutate(wherewhen = paste0("n_", year))
  ) 
stats_text  <- stats_text |> 
  mutate(T = round(T, 2), p = format.pval(p, digits = 2, eps = 0.001)) |> 
  purrr::transpose() |> 
  (\(x)set_names(x, stats_text$wherewhen))() |> 
...
Density discontinuity analysis [#Cattaneo2018] find so evidence of a discontinuity at the regulatory limit in British Columbia (`r stats_text$britishcolumbia`) ([@fig-rdd] A). 
...
rdds$britishcolumbia$plot$Estplot + 
  rdds$scotland$plot$Estplot +
  norway0.5_rdd$`2012`$plot$Estplot +
  norway0.5_rdd$`2020`$plot$Estplot +
...
First, as argued by [#Saevik2023] in response to [#Jeong2023], the sharp drop in lice numbers at the regulatory limit could be due to frequent and effective de-lousing treatments employed by the Norwegian aquaculture industry.
Such treatments will undoubtedly steepen the curve of the lice count distribution, especially close to and above the regulatory limit, but cannot explain the abrupt drop at the limit because as counting uncertainties would result in a smoother decline. 
This can be shown by simulating lice counts when the true counts are all strictly below the limit (@fig-count-uncertainty).
...
ggplot(counted, aes(x = value)) +
  geom_histogram(
    binwidth = 0.01,
    boundary = 0.005
  ) +
  geom_vline(xintercept = 0.5, colour = "#99151c", linetype = "dashed") +
  labs(x = "Simulated number of adult female lice per fish", 
       y = "Number of surveys") 
...
opt_stop_sim <- tibble(
  ...
  summarise(mean_original = mean(original), 
...
Norwegian regulations # require that counts are based on _at least_ twenty fish per pen (ten outside of the smolt migration period). 
...
To show the effect of optimal stopping, we run a simulation of `r nsim` lice counts. In each count, the lice have a Poisson distribution with a mean of `r true_mean` per fish. If the original count exceeds the regulatory limit of `r threshold` lice per fish, an extra `r nextra` fish are counted and the mean louse count recalculated.
In a simulation `r opt_stop_sim$thresh_original`% of the original counts exceed the limit, whereas `r opt_stop_sim$thresh_final`% of the counts after optimal stopping exceed the limit. 
...
Norwegian regulations require a louse count to be reported each week (every two weeks when the temperature is below 4˚C)
...
[#Godwin2020] showed that lice counts are almost twenty percent higher in audits than in routine counts.
...
All data are publicly available from the regulars in Norway, Scotland, and British Columbia. 
...
https://esajournals.onlinelibrary.wiley.com/doi/10.1002/eap.2226
...
FOR-2012-12-05-1140 https://lovdata.no/dokument/SF/forskrift/2012-12-05-1140
