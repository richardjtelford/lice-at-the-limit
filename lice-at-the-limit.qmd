...
There is a cost to compliance that companies will seek to minimise by developing efficient procedures, by subverting the regulations, or by breaching them if the penalties are low or enforcement is weak. 
If subversion is widespread, regulations may fail to reach their objectives even though all parties are apparently fully complying.
...
In temperate and sub-polar coastal waters, salmon farms are a major economic x with worldwide revenues of $xxxx, release xxx into the water. 
...
Jeong et al. [-@Jeong2023] ascribe this "cliff" to a desire by aquaculture managers to report values below the regulatory limits, but do not identify a mechanism by which they could do this. 
...
We test if there is a "cliff" or discontinuity in the louse counts distribution at the regulatory limit and try to explain it by natural processes or due to the regulations being subverted.
...
Lice count data were downloaded from the regulators in British Columbia, (Canada) (`r period$britishcolumbia`), Ireland (), Norway (`r period$norway`), and Scotland (`r period$scotland`).
...
Density discontinuity analysis [@Cattaneo2018] finds evidence of a discontinuity at the regulatory limit in British Columbia (`r stats_text$britishcolumbia`) ([@fig-rdd] A). 
...
Who uses barrentswatch data... check citations. classify into groups
