---
title: "Lice at the limits: impact of regulatory limits on salmon lice counts in aquaculture facilities"
author:
...
Companies will seek to minimise the cost of compliance by developing efficient procedures #, by circumventing the regulations #, fraudulently claiming to comply [@Jung2019], or by breaching them if the penalties are low or enforcement is weak #.
...
More onerous regulations further increase the cost of compliance and risk higher levels of non-compliance #.


In common with many polluting industries, emissions from the aquaculture industry are regulated.
Open-net pen salmon farms' release of pharmaceutical products [@CasaResino2021], heavy metals [@Burridge2010], and organic waste [@Elvines2023] into the environment is regulated ...
In addition and escaped fish cause genetic pollution of wild stock [@Glover2017].
In addition, farmed salmon host disease and parasites can be released into the environment and harm wild fish [@Krkosek2024].
...
tibble::tribble(
  ...
  "British Columbia, Canada", "????", "3 motile lice", "Yes, with audits",
...
The Norwegian regulations are the strictest, both having the lowest limit for female lice per fish, and the limit is the maximum permitted number of lice, whereas in Ireland and British Columbia, the limit is a trigger for treatment, and in Scotland the limit is a trigger for increased monitoring, and the trigger for treatment is 6 adult female lice per fish  [@Vormedal2023].
...
This analysis builds upon the work of Jeong et al. [-@Jeong2023] who reported that in Norway the distribution of louse counts has an unexpected "cliff" at the regulatory limit.
Data from British Columbia, Canada did not show this "cliff".
...
testing if there is a "cliff" or discontinuity in the louse counts distribution at the regulatory limit and try to explain it by natural processes or due to the regulations being circumvented. 
...
Lice count data were downloaded from the regulators in British Columbia, (Canada) (`r periods$britishColumbia`) [@FOC2024], Ireland (`r periods$ireland`) [@MarineInstitute20082024], Norway (`r periods$norway`) [@Barentswatch2024], and Scotland (`r periods$scotland`) [@SEPA2024].
...
The proportion of lice counts that are comply with the regulations [@fig-compliance] is high and increasing in Ireland (reaching 100% compliance in 2023) and Norway regardless of season, but somewhat lower and more varied in British Columbia.
...
```{r}
#| label: fig-t-stat
#| fig-cap: Magnitude of the discontinuity in salmon lice counts in Norway. THe magnitude is represented by the ratio between the density discontinuity smoother estimate immediately below of the regulatory limit and that to the above. From 2017, the smolt migration season has a lower limit and is shown separately. 
...
Sævik and Sandvik [-@Saevik2023] simulate lice population growth and treatment to generate data that resembles the data shown by Jeong et al. [-@Jeong2023].
In their simulation, once the lice abundance exceeds the regulatory limit, there is a 90% probability that the fish are cleaned with with an efficacy of 0.9. 
This is identical to the third mechanism we propose.
...
The size of the discontinuity in louse counts suggests that reporting post-treatment counts is widespread in the Norwegian aquaculture industry to the extent that it are standard practice.
...
The Norwegian sea lice law [-@sealicelaw2012] § 8 states that "at any time there must be fewer than [0.2 or 0.5 depending on the season] adult female salmon lice on average per fish in the aquaculture facility".
The apparently pervasive practice of recounting the lice after cleaning if an initial count is above the limit implies that Norwegian aquaculture facilities are routinely out of compliance with the law and the lice counts they submit misrepresent the true abundance of lice.
The effect this reporting procedure on estimates of lice abundance can be demonstrated by taking some 2013 lice counts and reporting them as they would be in later years [@fig-2013plot].
...
Van Nes et al [-@Nes2024] presents a critical review of the traffic light system, arguing that several aspect of current practice have large uncertainties or overestimate the effect of lice on wild salmon including estimates of farm origin lice infestation pressure.
Van Nes et al [-@Nes2024] argue that the low lice counts typically reported imply that only a small proportion of the female lice will be reproductively active. 
This is density dependent effect, with low louse densities causing low encounter rates between adult male and female lice so not all females are fertilised [@Groner2014].
...
That Norway is the only country with a large discontinuity in louse counts at the regulatory limit suggests that the difference is due to the limit being a maximum permitted count in Norway and a trigger for treatment (or enhanced monitoring) in the other countries, coupled with the low limits in Norway. 
Previously, before the discontinuity developed, the Norwegian regulations [@sealicelaw2000] specified a limit, above which mandatory delousing treatment was required.
Returning to this previous rule reduce the incentive to circumvent the limit.
On paper, this change would make the regulations less stringent.
In practice, if the trigger for treatment was strictly enforced, the situation would be identical to the current status, but the louse count data would more reliable and hence more useful for managing the impact of salmon farming on wild salmon populations.


## Conclusions

Circumvention of the regulations at Norwegian aquaculture facilities allow them to report that they are in compliance with the regulations when the regulatory limit on the number of lice has been exceeded.
This circumvention began after the regulations were changed to make the limit a the maximum permitted lice count, rather than a threshold for mandatory treatment.
Reverting to this previous policy would reduce the incentive to circumvent the regulations, making the louse count data more reliable, without affecting the actual number of lice.
